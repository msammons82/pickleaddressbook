...
class AddressBook:
    ...
    def initialization(self):
        ...
        self.status_bar = Label(main_frame, text=" Pickle Address Book ",relief = SUNKEN, bd=1,foreground='blue').pack(side=BOTTOM, fill= X)

        frame1 = Frame(main_frame, bd=50,background='grey')
        frame1.pack(fill=BOTH, expand = YES, side=LEFT)

        scroll = ttk.Scrollbar(frame1, orient=VERTICAL)
        self.lbx_names = Listbox(frame1, width=50,height=50, yscrollcommand=scroll.set,foreground='red')

        self.lbx_names.pack(fill=Y, side=LEFT)
        scroll.config(command=self.lbx_names.yview)
        scroll.pack(side=LEFT, fill=Y)

        frame2 = Frame(main_frame, bd=30,background='greenyellow')
    ...
    def save_address(self):
        ...
        tkinter.messagebox.showinfo("Address Saved", "A new address has been saved, wow you have no friends")
        outfile.close()

    def modify_address(self):
        outfile = open("address.dat", "wb")
        pickle.dump(self.lst_addresses, outfile)
        tkinter.messagebox.showinfo("Address Changed", "The changes have been saved, maybe???")
        outfile.close()

    def delete_address(self):
        outfile = open("address.dat", "wb")
        pickle.dump(self.lst_addresses, outfile)
        tkinter.messagebox.showinfo("Address Deleted", "The address has been deleted forever,muhahahhaha!")
        outfile.close()
...
if __name__ == '__main__':
    ...
    application = AddressBook(root, "Pickle Address Book")
